...
# Every pattern is the canonical one, unchanged. The one delta:
#
#   - The projected ServiceAccount token mount,
#     /var/run/secrets/kubernetes.io/serviceaccount/, is a Kubernetes path
#     the responder reads to review its own token: mechanism, not a
#     secret path of any estate. The '/secrets/' pattern (SSM parameter
#     paths) is kept at full strength and a line is exempt only when it
#     carries that exact literal — see `mechanism` below.
#
#
...
# Known mechanism, exempted by exact literal (see the header). Nothing
# else is: a pattern hit on any other text is a leak.

...
func TestParseServiceAccount(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			username:      "system:serviceaccount:my-namespace:worker",
		...
		},
		{
			...
			username: "system:serviceaccount:my-namespace",
			wantErr:  true,
		},
		{
...
func TestAccountForNamespace(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "listed project maps to itself", namespace: "my-namespace", wantAccount: "my-namespace"},
		{name: "other listed project maps to itself", namespace: "billing", wantAccount: "billing"},
		{name: "employee namespace maps to itself", namespace: "emp-example", wantAccount: "emp-example"},
		...
		{name: "ci tenant with dashed repo maps to itself", namespace: "ci-truvity-my-namespace", wantAccount: "ci-truvity-my-namespace"},
		...
		{name: "retired employee- prefix rejected", namespace: "employee-example", wantErr: true},
...
func TestAccountForNamespaceNoProjects(t *testing.T) {
	if _, err := AccountForNamespace("my-namespace", nil); err == nil {
		t.Error("AccountForNamespace with empty project list should reject a project namespace")
	}

	account, err := AccountForNamespace("emp-example", nil)
	if err != nil || account != "emp-example" {
		t.Errorf("AccountForNamespace(emp-example, nil) = (%q, %v), want (emp-example, nil)", account, err)
	}
}
